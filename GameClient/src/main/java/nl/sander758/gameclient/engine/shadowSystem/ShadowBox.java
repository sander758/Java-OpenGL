...
public class ShadowBox {
	...
	protected void update() {
        ...
        Vector4f rotationVector = rotation.transform(FORWARD);
		Vector3f forwardVector = new Vector3f(rotationVector.x, rotationVector.y, rotationVector.z);
	...
	protected Vector3f getCenter() {
        ...
        Vector4f invertedLightVector = invertedLight.transform(cen);
		return new Vector3f(invertedLightVector.x, invertedLightVector.y, invertedLightVector.z);
	}
	...
	private void calculateWidthsAndHeights() {
		farWidth = (float) (SHADOW_DISTANCE * Math.tan(Math.toRadians(Maths.FOV)));
		nearWidth = (float) (Maths.NEAR_PLANE
				* Math.tan(Math.toRadians(Maths.FOV)));
	...
	private float getAspectRatio() {
		return (float) WindowManager.getWidth() / (float) WindowManager.getHeight();
	}
